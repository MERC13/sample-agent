...
# Ollama import and availability check
try:
...
# Allowed imports and builtins for safe execution
import ast
import io
import contextlib
...
def validate_a2a_message(msg: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    for idx, p in enumerate(parts):
        ...
        # Other kinds (file, image, etc.) are tolerated but not strictly validated here.
    return True, ""
...
def handle_message(text: str, params: dict, message_id: str | None = None) -> Dict[str, Any]:
    ...
    if OLLAMA_AVAILABLE:
        ...
        resp_text = "Ollama model not available."
...
def process_request():
    ...
    if method == "message/send":
        ...
        ok, err = validate_a2a_message(message)  # type: ignore[arg-type]
        if not ok:
        ...
        message_id = message.get("messageId")  # type: ignore[assignment]
        for part in message.get("parts", []):  # type: ignore[union-attr]
